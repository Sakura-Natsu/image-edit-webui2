...
export async function POST(req: Request) {
  ...
  if (!apiKey) {
    console.warn("[api/generate] 缺少 API Key");
  ...
  const modelHint = (body as { model?: string } | null)?.model ?? "?";
  console.log(`[api/generate] → ${target} model=${modelHint} key-source=${source}`);
  ...
  try {
    ...
    if (!upstream.ok) {
      console.error(
        `[api/generate] ← ${upstream.status} in ${dur}ms`,
        text.slice(0, 500),
      );
    } else {
      console.log(`[api/generate] ← ${upstream.status} in ${dur}ms (${text.length}B)`);
    }
    ...
    console.error("[api/generate] 请求上游异常", err);
